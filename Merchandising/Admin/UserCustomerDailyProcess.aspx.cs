...
{
    public partial class UserCustomerDailyProcess : System.Web.UI.Page
    {
        ...
        public string mainConditions(string rotID, string cusID)
        {
            ...
            string mainCondition = "cus_rot_ID  = (case when '" + rotID + "' = '' then cus_rot_ID else '" + rotID + "' end)  and ucp_cus_ID = (case when '" + cusID + "' = '' then ucp_cus_ID else '" + cusID + "' end)";
            try
            {
        ...
        public void ListData()
        {
            ...
            lstUser = ObjclsFrms.loadList("UserCustomerDailyProcess", "sp_Masters", condition.ToString());
        ...
        private void SpreadStreamProcessingForXLSXAndCSV(DataTable dt, SpreadDocumentFormat docFormat = SpreadDocumentFormat.Xlsx, string sheetName = "Sheet1")
        {
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                Response.AppendHeader("Content-Disposition", string.Format("attachment; filename={0}.{1}", "User Customer Daily Process", "Xlsx"));
