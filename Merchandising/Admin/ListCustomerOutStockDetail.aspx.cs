...
namespace Merchandising.Admin
{
    public partial class ListCustomerOutStockDetail : System.Web.UI.Page
    {
        ...
        {
            string[] arr = { Session["FromDate"].ToString(), Session["ToDate"].ToString() };
            DataTable lstUser = default(DataTable);
            if(Mode == 0)
            {
                lstUser = ObjclsFrms.loadList("SelectCustomerOutOfStockDetail", "sp_Report", ResponseID.ToString(), arr);
            }
            else
            {
                lstUser = ObjclsFrms.loadList("SelectItemOutOfStockDetail", "sp_Report", ResponseID.ToString(), arr);
            }
        ...
        public void CustomerHeader()
        {
            ...
            lstDatas = ObjclsFrms.loadList("SelectCustomerOutOfStockHeader", "sp_Report", ResponseID.ToString());
        ...
        {
            DataTable lstDatas = new DataTable();
            lstDatas = ObjclsFrms.loadList("SelectItemOutOfStockHeader", "sp_Report", ResponseID.ToString());
            if (lstDatas.Rows.Count > 0)
            {
                ...
                lblDescription.Text = lstDatas.Rows[0]["prd_Desc"].ToString();
