    ...
    public partial class CustomerTask : System.Web.UI.Page
    {
        ...
        public string mainConditions(string rotID)
        {
            ...
            try
            {
                ...
                dateCondition = " and (isnull(cast(A.cst_CapturedTime as date),cast(A.CreatedDate as date)) between cast('" + fromDate + "' as date) and cast('" + endDate + "' as date))" ;
                if (cusID.Equals("0"))
                {
                    customerCondition = "";
                }
            ...
            if (!rotID.Equals("0"))
            {
                ...
                if (Mode == 1)
                {
                    try
                    {
                        ...
                        {
                            mainCondition += " and A.cst_Status='C'";
                        }
                ...
                DataTable lstdata = ObjclsFrms.loadList("SelectCustomerTask", "sp_Transaction", mainCondition);
        ...
        protected void ImageButton4_Click(object sender, ImageClickEventArgs e)
        {
            ...
            foreach (GridColumn column in grvRpt.MasterTableView.Columns)
            {
                if (!string.IsNullOrEmpty(column.UniqueName)
                   ...
                   && !column.HeaderText.Equals("Edit") && !column.HeaderText.Equals("Reference Image") && !column.HeaderText.Equals("Captured Image")
                    )
                {
            ...
            foreach (GridDataItem item in dd.MasterTableView.Items)
            {
                ...
                for (int i = 1; i < grvRpt.MasterTableView.Columns.Count; i++)
                {
                    if (grvRpt.MasterTableView.Columns[i].Display == true)
                    {
                        ...
                        {
                            if (!grvRpt.MasterTableView.Columns[i].HeaderText.Equals("Reference Image") && !grvRpt.MasterTableView.Columns[i].HeaderText.Equals("Captured Image"))
                            {
