    ...
    public partial class AddEditCusHeader : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                {
                    DataTable lstDatas = obj.loadList("EditCusHeader", "sp_Masters_Temp", ResponseID.ToString());
                    if (lstDatas.Rows.Count > 0)
                    {
                        string Cshname, Cshcode;                                          //Declare the variables
        ...
        public void Save()
        {
            ...
            {
                string[] arr = { Cshcode};
                string value = obj.SaveData("sp_Masters_Temp", "InsertCusHeader", Cshname, arr);
                int res = Int32.Parse(value.ToString());
                if (res > 0)
                {
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "tmp", "<script type='text/javascript'>Succcess('Customer Header Saved Successfully');</script>", false);
                }
                else
                {
            ...
            {
                string ID = ResponseID.ToString();
                string[] arr = { Cshcode, ID };
                string value = obj.SaveData("sp_Masters_Temp", "UpdateCusHeader", Cshname, arr);
                int res = Int32.Parse(value.ToString());
                if (res > 0)
                {
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "tmp", "<script type='text/javascript'>Succcess('Customer Header updated successfully');</script>", false);
                }
                else
                {
                    ScriptManager.RegisterStartupScript(this.Page, this.GetType(), "tmp", "<script type='text/javascript'>Failure();</script>", false);
        ...
        {
            Response.Redirect("/BO_Digits/en/ListCustomerHeader.aspx");
        ...
        protected void txtCode_TextChanged(object sender, EventArgs e)
        {
            ...
            DataTable lstCodeChecker = obj.loadList("CheckCusHeaderCode", "sp_CodeChecker", code);
