...
namespace Merchandising.BO_Digits.en
{
    public partial class AddEditCustomerBrochure : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                txtFrom.MinDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day);
                txtToDate.MinDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day);
                Route();
            }
        }
        public void Route()
        {
            DataTable lstType = ObjclsFrms.loadList("DropDownRouteForBrochure", "sp_Masters");
        ...
        {
            ddlCustomers.DataSource = ObjclsFrms.loadList("SelectCustomerForBrochure", "sp_Masters", ddlRoute.SelectedValue.ToString());
        ...
        protected void Save()
        {
            ...
            foreach (UploadedFile uploadedFile in upd1.UploadedFiles)
            {
                ...
                string csvPath = Server.MapPath(("..") + @"/../UploadFiles/Brochure/") + ImageID.ToString() + "_" + uploadedFile.FileName;
                uploadedFile.SaveAs(csvPath);
                image = @"/UploadFiles/Brochure/" + ImageID.ToString() + "_" + uploadedFile.FileName.ToString();
                ViewState["image"] = image.ToString();
            }

            string cust, name, from, to, path, user, rot,arabic;
            ...
            string Value = "";
            string[] arr = { name.ToString(), from.ToString(), to.ToString(), path.ToString(), user.ToString(), rot ,arabic};
        ...
        protected void lnkSave_Click(object sender, EventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                ObjclsFrms.LogMessageToFile(UICommon.GetLogFileName(), "AddEditCustomerBrochure.aspx PageLoad()", "Error : " + ex.Message.ToString() + " - " + innerMessage);
            }
        }
