...
namespace Merchandising.BO_Digits.en
{
    public partial class AddEditCustomerTasks : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                date.MinDate = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day);
                Route();
               // Customer();
                Brand();
            }
        ...
        {
            DataTable lstType = ob.loadList("SelectCustomer", "sp_CustomerTasks",ddlRoute.SelectedValue.ToString());
            if (lstType.Rows.Count > 0)
            {
