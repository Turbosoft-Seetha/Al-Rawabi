    ...
    public partial class ListCustomerAssetTrackingDetail : System.Web.UI.Page
    {
        ...
        private void SpreadStreamProcessingForXLSXAndCSV(DataTable dt, SpreadDocumentFormat docFormat = SpreadDocumentFormat.Xlsx, string sheetName = "Sheet1")
        {
            using (MemoryStream stream = new MemoryStream())
            {
                ...
                Response.AppendHeader("Content-Disposition", string.Format("attachment; filename={0}.{1}", "Customer Asset Tracking Detail", "Xlsx"));
                ...
                if (dataItem["qst_Name"].Text == "Image")
                {
                    ...
                    for (int i = 0; i < values.Length; i++)
                    {
                        if (!values[i].Equals("&nbsp;") && !values[i].Equals(""))
                        {
                            ...
                            string im = dataItem["Images"].Text.ToString();
                        }
                    }
                }
                else
